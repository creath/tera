TeraCoin-qt: Qt4 GUI for TeraCoin
===============================
...
Alternatively, install Qt Creator and open the `TeraCoin-qt.pro` file.

An executable named `TeraCoin-qt` will be built.
...
A warning for people using the *static binary* version of TeraCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of TeraCoin is linked against libdb5.3.
...
installed causes TeraCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
