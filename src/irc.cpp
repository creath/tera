...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #TeraCoinTEST2\r");
            Send(hSocket, "WHO #TeraCoinTEST2\r");
        } else {
            // randomly join #TeraCoin00-#TeraCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #TeraCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #TeraCoin%02d\r", channel_number).c_str());
        }
